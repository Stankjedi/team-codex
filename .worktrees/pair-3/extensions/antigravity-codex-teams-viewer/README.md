...
- 확장은 **통합 터미널을 새로 열지 않고** 백그라운드 명령 실행 결과만 웹뷰에 렌더링
- `codex-teams`가 생성하는 `.codex-teams/.viewer-session.json` 브리지를 자동 감지해 현재 세션을 따라감
...
- `agCodexTeamsViewer.useSkillBridge` (default: `true`)
- `agCodexTeamsViewer.skillBridgePath` (default: `${workspaceFolder}/.codex-teams/.viewer-session.json`)
...
- `agCodexTeamsViewer.wslDistro` (Windows에서 특정 WSL 배포판 강제)
